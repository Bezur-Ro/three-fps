import * as THREE from '../game/three/three.module.min.js';
...
function init() {
  ...
  blockerContents.addEventListener( 'click', function () {
    var isFocused = (document.activeElement === document.getElementById('name'));
  ...
  controls.addEventListener( 'lock', function () {
    ...
    if(socket == null) {
      ...
      socket.on('shoot', function(msg) {
        if(msg.length == 1) {
          var bullet = scene.getObjectByName("bullet" + msg[0]);
          scene.remove(bullet)
        }
        else {
          var bullet = scene.getObjectByName("bullet" + msg[4]);
          if(bullet == null) {
            createBullet(msg)
          }
          else {
  ...
  const onKeyDown = function ( event ) {
    ...
    if(typingAMessage) {
      if(event.keyCode == 191) {
        ...
        var expression = new RegExp("^[A-Z0-9_ ]*$")
...
function animate() {
  ...
  if ( controls.isLocked === true && socket != null) {
    ...
    var pos = controlsObject.position
    var rotation = controlsObject.rotation
